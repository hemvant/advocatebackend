...
async function createPackage(req, res, next) {
  try {
    const { name, description, price_monthly, price_annual, annual_discount_percent, employee_limit, module_ids } = req.body;
    ...
    const pkg = await Package.create({
      ...
      price_annual: Number(price_annual),
...
async function updatePackage(req, res, next) {
  try {
    const pkg = await Package.findByPk(req.params.id);
    if (!pkg) return res.status(404).json({ success: false, message: 'Package not found' });
    const { name, description, price_monthly, price_annual, annual_discount_percent, employee_limit, is_active, module_ids } = req.body;
